...
const Arrivals = () => {
  ...
  const {
    ...
    data: arrivalsData,
    error: arrivalsError,
  } = useQuery<{ data: Booking[] | BookingSumm[] }, Error>(["bookings", { guests: { some: { start: new Date(date!).toISOString()}}}], () =>
    getBookings({
      guests: {
        some: {
          start: new Date(date as Date).toISOString(),
        },
      },
            ...
            <AccordionDetails>
              <div id="summary-blocks">
                ...
                {selectedSite?.guestTypeGroups?.map(guestTypeGroup => {
                  ...
                  const numberOfTypeDue = calculateNumberOfType(guestTypeGroupId, arrivalsData!.data as Booking[], "DUE")
                  const numberOfTypeArrived = calculateNumberOfType(guestTypeGroupId, arrivalsData!.data as Booking[], "ARRIVED")
              ...
              >
                <ArrivalsGraph
                  arrivalsData={arrivalsData!.data as Booking[]}
                  height={300}
                  width={500}
                />
              </div>
              ...
              <TableContainer>
                <Table aria-label="simple table">
                  ...
                  <TableBody>
                    ...
                    {arrivalsData &&
                      (arrivalsData.data as Booking[]).map((arrival) => {
